...
final class HelperTargetPackagingTests: XCTestCase {
    func testHelperTargetIsAnEmbeddedAgentAppWithoutHostOnlyServices() throws {
        ...
        XCTAssertTrue(helperSource.contains("windowDidBecomeKey"))
        XCTAssertTrue(helperSource.contains("windowShouldBecomeKey"))
        XCTAssertTrue(helperSource.contains("current.activate(from: frontmost"))
        XCTAssertTrue(helperSource.contains("yieldActivation(to: host)"))
