...
The estimate does not include hosted-tool call charges, regional-processing uplifts, long-context multipliers, Batch/Flex discounts, negotiated rates, credits, taxes, or ChatGPT subscription terms. Fast/Priority pricing is included when the rollout reports that service tier. When Wei-Shaw/sub2api is selected, its current-day usage log overrides the local estimate by `session_id`; this captures server-side tier/effort rewrites and Free Fast billing that Codex rollouts cannot observe. **It is not an invoice.** Actual API spend should come from the OpenAI Organization Costs endpoint and may only be attributable to OpenAI API project IDs, not local filesystem projects.
